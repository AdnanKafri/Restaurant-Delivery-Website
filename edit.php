...
{      // $id=$_POST["me_ID"]
    ...
    $photo=$_POST['photo'];
    $sql="UPDATE `meal` SET me_name=?,description=?,price=?,re_name=?,re_phone=?,
          photo=? WHERE me_ID='$mid'";
    $ps = $pdo->prepare($sql);
...
<body>
<div class="container">
    <form action="edit.php<?php echo "?id=$id&mid=$mid";  ?>" method="post">
        <div class="form-group">
            <label for="me_name">اسم الوجبة الجديد</label>
            <input type="text" id="me_name" name="me_name" required>
        </div>
        <div class="form-group">
            <label for="description">وصف الوجبة الجديد</label>
            <textarea id="description" name="description" required></textarea>
        </div>
        <div class="form-group">
            <label for="price">سعر الوجبة الجديد</label>
        ...
        <div class="form-group">
            <button type="submit"><a href="edit.php?id=$id" class="btn"></a>Submit</button>
        </div>
